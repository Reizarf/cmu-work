...
int shellSort(vector<int> &arr)
{
    ...
    while(interval != 0)
    {
        ...
        {
            for(int p = i + interval; p < arr.size(); p+= interval)
            {
                int key = arr[p];
                int j = p - interval;
                while(j >= 0)
...
// void mergE(vector<int> &arr,int start, int middle, int end,int &comparisons)
// {
...
// 		mergeSort(arr,start,end,comparisons);//sort first half
// 		mergeSort(arr,start,end,comparisons);//sort second half
// 		//merge sorted halves
